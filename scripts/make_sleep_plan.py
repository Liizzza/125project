...
def estimate_sleep_debt(df_recent: pd.DataFrame, target_min: int) -> float:
    ...
    Also caps single-night deficit to avoid outliers dominating.
    """
    last7 = df_recent.tail(7).copy()
    weights = [1.0, 1.25, 1.5, 1.75, 1.85, 1.95, 2.0]
    weights = weights[-len(last7):]
    total_debt = 0.0
    total_weight = 0.0
    for (_, row), w in zip(last7.iterrows(), weights):
        sleep = float(row["total_sleep_min"])
        deficit = max(0.0, min(target_min * 2, target_min - sleep))
        total_debt += deficit * w
        total_weight += w
    raw = total_debt / total_weight * min(len(last7), 7)
    return float(raw)
...
def bedtime_score(bed_wrap, baseline_bed_wrap, mins_until_bed, debt_min):
    ...
    not earlier. This lets the algorithm push bedtime earlier without penalty.
    """
    if debt_min > 300:
...
def score_plan(now_min, bed_min, wake_min, baseline, constraints, desired_sleep_min, debt_min):
    ...
    # Asymmetric wake penalty: being early is less bad than being late
    ...
    # Boost sleep weight when debt is high
...
def desired_sleep_from_debt(target_min, debt_min):
    """
    Gradual debt recovery curve:
    Low (<60): no extra | Medium (60-300): +45 max | High (300-600): +90 max | Very high: +120 max
    """
    if debt_min < 60:
        extra = 0
    elif debt_min < 300:
